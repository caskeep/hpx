...
namespace hpx { namespace traits {
            ...
            communication_operation<Communicator, test::communication_set_tag>>
    {
        ...
        template <typename Result>
        Result get(std::size_t which, test::get_num_connected)
        {
            return communicator_.num_connected_;
        }
...
namespace test {
    ...
    using get_num_connected_action =
        typename hpx::lcos::detail::communication_set_node::template get_action<
            communication_set_tag, std::size_t, get_num_connected>;
...
std::size_t num_connected_nodes[] = {
    0,  0,  0,  4,  0,  8,  8, 12,   //  0.. 7
    0, 16, 16, 20, 16, 24, 24, 28,   //  8..15
    0, 32, 32, 36, 32, 40, 40, 44,   // 16..23
   32, 48, 48, 52, 48, 56, 56, 60,   // 24..31
...
void test_communication_set(std::size_t size, std::size_t arity)
{
    ...
    // verify how many nodes are connected to a given one
    //     for (std::size_t i = 0; i != size; ++i)
    //     {
    //         hpx::future<std::size_t> f =
    //             hpx::async(test::get_num_connected_action{}, node_ids[i],
    //                 i, test::get_num_connected{});
    //         HPX_TEST_EQ(f.get(), num_connected_nodes[i / arity]);
    //     }
}
