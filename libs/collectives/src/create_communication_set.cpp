...
namespace hpx { namespace lcos {
    ...
    namespace detail {
        ...
        std::size_t next_power_of_two(std::uint64_t v)
        {
            --v;
            v |= v >> 1;
            v |= v >> 2;
        ...
        std::size_t num_sites, std::size_t this_site, std::size_t arity)
    {
        ...
        {
            arity = 2;
            // std::stoull(get_config_entry("hpx.lcos.collectives.arity", "2"));
        }

        // the arity has to be a power of two
        HPX_ASSERT(detail::next_power_of_two(arity) == arity);
        ...
        {
            node_id = hpx::find_from_basename(name, this_site - 1);
        }
